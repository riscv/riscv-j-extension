...
[NOTE]
====
If N is less than or equal to NVBITS for the current address translation mode, this is equivalent to ignoring a subset of NVBITS. This enables cheap implementations that disable validity checks in the MMU instead of performing the sign extension.
====
...
Table 1 shows an example of the pointer masking transformation on a virtual address when PM is enabled for RV64 under Sv57 (N=7).
...
|N+1st bit from the top (i.e., bit XLEN-N)|1
|Transformed virtual address |0xFFFFFFFF12345678 +
NVBITS[1111111]  VBITS[11111111111111111111111110001...000]

|===

If the address was a physical address rather than a virtual address with Sv57, the transformed address with N=7 would be 0x1FFFFFF12345678.
...
The specified extensions **do not** support N to be configurable and the current standard only supports N=XLEN-48 (i.e., N=16 in RV64). A field has been reserved to potentially support different values of N in future standards. In such future standards, different values of N may be defined for each privilege mode (U/VU, S/HS, and M) and for different active address translation modes.
