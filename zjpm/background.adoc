...
* **Hypervisor Extension**: HLV.\*, HSV.* (if enabled via `hstatus.PMM`, see <<_ssnpm>>).
...
MPRV and SPVP affect pointer masking as well, causing the pointer masking settings of the effective privilege mode to be applied. The exception to this rule is the case where MPRV is set while MXR is also set, in which case pointer masking does not apply (see <<_ssnpm>> for the rationale).
