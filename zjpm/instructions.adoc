...
In systems where the H Extension is present, `Ssnpm` also adds a new 2-bit WARL field (`PMM`) to bits 33:32 of `henvcfg`. Setting `PMM` enables or disables pointer masking for VS-mode, according to the values in Table 2. Further, a 2-bit WARL field (`PMM`) is added to bits 49:48 of `hstatus`. Setting `hstatus.PMM` enables or disables pointer masking for `HLV.\*` and `HSV.*` instructions in U-mode, according to the values in Table 2. In HS- and M-mode, pointer masking for these instructions is enabled or disabled by `senvcfg.PMM`.
